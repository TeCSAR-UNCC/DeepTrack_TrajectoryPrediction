...
from ATCN.net import get_network
...
class highwayNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        # --------------For history
        ATCN_DeepTrack_sequencer = {  # Half output chanel size
            'nhid': [32, 16, 64],
            'sdil': [1,  2,   4],
            'skrn': [8, 4, 2],
            'input_scaling': [1]*3,
        ...
        self.atcn_hrst = get_network(
            ATCN_DeepTrack_sequencer,  self.in_length,  self.out_length, 0.25, use_conv2d=True, first_layer_chn=2)

        # --------------For neighbors
        ATCN = {  # Half output chanel size
            'nhid': [32, 16, 64],
            'sdil': [1,  2,   4],
            'skrn': [8, 4, 2],
            'input_scaling': [1, 1, 1],
        ...
        self.atcn_nbr = get_network(
            ATCN, self.in_length, self.out_length, 0.25, use_conv2d=True, first_layer_chn=2)
        # -----------------------------
    ...
    def forward(self, hist, nbrs, masks, lat_enc, lon_enc):
        ...
        x_hrst = hist.permute(1, 2, 0).unsqueeze(2)
        x_hrst = self.atcn_hrst(x_hrst)
        hist_enc = torch.flatten(x_hrst, 2)

        hist_enc = hist_enc.permute(0, 2, 1)
        lstm_weight = self.pre4att(self.tanh(hist_enc))  # lstm_out))
        new_hidden, soft_attn_weights = self.attention(
            lstm_weight, hist_enc)  # lstm_out)
        ...
        x_nbr = nbrs.permute(1, 2, 0).unsqueeze(2)
        x_nbr = self.atcn_nbr(x_nbr)
        nbrs_enc = torch.flatten(x_nbr, 2)

        nbrs_enc = nbrs_enc.permute(0, 2, 1)
        nbrs_lstm_weight = self.pre4att(self.tanh(nbrs_enc))

        new_nbrs_hidden, soft_nbrs_attn_weights = self.attention(
            nbrs_lstm_weight, nbrs_enc)
        nbrs_enc = new_nbrs_hidden

        # Masked scatter
        # mask size: (128, 3, 13, 64)
        ...
        soc_enc = soc_enc.contiguous().view(
            soc_enc.shape[0], soc_enc.shape[1], -1)
        ...
        new_hidden_ha, soft_attn_weights_ha = self.attention(
            weight, new_hs_per)
